  ...
  return (
    <div className="space-y-8 animate-in pb-24 print:p-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 print:hidden">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic">วิเคราะห์ข้อมูล <span className="text-purple-600">Analytics</span></h1>
      ...
      <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-xl space-y-8">
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 print:hidden">
            <div className="flex bg-slate-100 p-1.5 rounded-2xl">
               {(['service', 'inventory', 'billing'] as const).map(type => (
                 <button key={type} onClick={() => setReportType(type)} className={`px-10 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${reportType === type ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <select className="pl-10 pr-6 py-3 bg-slate-50 border-none rounded-xl font-black text-[9px] uppercase outline-none">
         ...
         <div className="overflow-x-auto min-h-[300px]">
            <table className="w-full text-left">
               <thead>
                  <tr className="border-b border-slate-100 text-