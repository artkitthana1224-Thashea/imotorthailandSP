...
import { 
  ...
  Activity, Calendar, Package, MapPin, Phone, User, Save, UserPlus, AlertCircle, ExternalLink
} from 'lucide-react';
...
export const WorkOrderView: React.FC<WorkOrderViewProps> = ({ workOrders, setWorkOrders, vehicles }) => {
  ...
  const handleSaveOrder = async () => {
    ...
    if (showNewCustomerForm) {
      const { data: newCust } = await supabase.from('customers').insert([{ id: Date.now().toString(), name: newCustomerData.name, phone: newCustomerData.phone, address: newCustomerData.address, province: newCustomerData.province, company_id: '1001', type: 'INDIVIDUAL' }]).select().single();
      if (newCust) finalCustomerId = newCust.id;
      await fetchCustomers();
    }
    const orderData = { order_number: formData.orderNumber, customer_id: finalCustomerId, vehicle_id: formData.vehicle_id || null, status: formData.status, issue_description: formData.issue_description, labor_cost: formData.labor_cost, total_amount: formData.total_amount, company_id: '1001' };
    const { error } = selectedOrder ? await supabase.from('work_orders').update(orderData).eq('id', selectedOrder.id) : await supabase.from('work_orders').insert([{ ...orderData, id: Date.now().toString() }]);
    if (!error) {
      await fetchWorkOrders();
      setIsModalOpen(false);
      const cust = customers.find(c => c.id === finalCustomerId) || newCustomerData;
      await sendWorkOrderNotification({ orderNumber: formData.orderNumber || '', status: formData.status || 'PENDING', customerName: cust.name, issue: formData.issue_description || 'ไม่มีรายละเอียด', creator: 'Admin', isUpdate: !!selectedOrder });
    }
    setIsSaving(false);
  };
  ...
  return (
    <div className="space-y-8 animate-in duration-700 pb-24">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button onClick={() => { setFormData({ orderNumber: `WO-${Date.now().toString().slice(-6)}`, status: WorkOrderStatus.PENDING, labor_cost: 500, total_amount: 500, customer_id: '' }); setSelectedOrder(null); setShowNewCustomerForm(false); setIsModalOpen(true); }} className="h-12 px-8 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg active:scale-95 transition-all">
          + เปิดใบงานใหม่
        </button>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-3xl rounded-[40px] shadow-2xl animate-in overflow-hidden flex flex-col">
              <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                 <h3 className="text-xl font-black uppercase italic tracking-tighter">Repair Work Order</h3>
              ...
              <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                       <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase text-slate-400 ml-1">ข้อมูลลูกค้า</label>
                       ...
                       <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase text-slate-400 ml-1">อาการเสีย</label>
                          <textarea value={formData.issue_description} onChange={e => setFormData({...formData, issue_description: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl h-24 text-xs font-black outline-none" />
                       </div>
                    </div>

                    <div className="space-y-4">
                       <label className="text-[10px] font-black uppercase text-slate-400 ml-1">พิกัดจุดนัดหมาย/ที่อยู่ลูกค้า</label>
                       {selectedCustomer?.address ? (
                         <div className="relative group overflow-hidden rounded-[32px] border border-slate-100 shadow-inner h-48 bg-slate-50">
                            <iframe 
                              width="100%" height="100%" frameBorder="0" style={{ border: 0 }}
                              src={`https://www.google.com/maps/embed/v1/place?key=${process.env.API_KEY}&q=${encodeURIComponent(selectedCustomer.address + ' ' + selectedCustomer.province)}`}
                              allowFullScreen
                            ></iframe>
                            <a 
                              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedCustomer.address)}`} 
                              target="_blank" 
                              className="absolute bottom-4 right-4 p-3 bg-white text-blue-600 rounded-2xl shadow-xl flex items-center gap-2 text-[10px] font-black uppercase opacity-0 group-hover:opacity-100 transition-all"
                            >
                               <MapPin size={14}/> เปิดในแอป Maps <ExternalLink size={12}/>
                            </a>
                         </div>
                       ) : (
                         <div className="h-48 border-2 border-dashed border-slate-100 rounded-[32px] flex flex-col items-center justify-center text-center p-8 text-slate-300">
                            <MapPin size={32} className="mb-2 opacity-20"/>
                            <p className="text-[9px] font-black uppercase">กรุณาเลือกลูกค้าที่มีข้อมูลที่อยู่</p>
                         </div>
                       )}
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-2 gap-4 pt-4">
                    ...
                    <div className="space-y-2 text-right">
                       <label className="text-[10px] font-black uppercase text-slate-400">ยอดสุทธิ (฿)</label>
              ...
              <div className="p-8 bg-slate-50 flex justify-end">
                 <button onClick={handleSaveOrder} disabled={isSaving} className="px-12 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center gap-3">
                    {isSaving ? "Saving..." : <><Save size={16}/> บันทึกข้อมูลใบงาน</>}
                 </button>
              </div>
           </div>
        </div>
